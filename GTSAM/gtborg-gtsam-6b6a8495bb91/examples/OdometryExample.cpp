...
int main(int argc, char** argv) {
  ...
  Pose2 priorMean(10.0, 10.0, 0.0); // prior at origin
  noiseModel::Diagonal::shared_ptr priorNoise = noiseModel::Diagonal::Sigmas(Vector3(0.3, 0.3, 0.1));
  graph.emplace_shared<PriorFactor<Pose2> >(1, priorMean, priorNoise);

  // Add odometry factors
  Pose2 odometry(3.0, 0.0, 0.0);
  Pose2 odometry1(-3.0, 4.0, 0.0);
  Pose2 odometry2(0.0, 4.0, 0.0);
  // For simplicity, we will use the same noise model for each odometry factor
  noiseModel::Diagonal::shared_ptr odometryNoise = noiseModel::Diagonal::Sigmas(Vector3(0.2, 0.2, 0.1));
  // Create odometry (Between) factors between consecutive poses
  graph.emplace_shared<BetweenFactor<Pose2> >(1, 2, odometry, odometryNoise);
  graph.emplace_shared<BetweenFactor<Pose2> >(2, 3, odometry1, odometryNoise);
  graph.emplace_shared<BetweenFactor<Pose2> >(1, 3, odometry2, odometryNoise);
  graph.print("\nFactor Graph:\n"); // print
